    ...
    <div className="border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] font-bold tracking-[0.12em] uppercase" style={{ color: "rgba(255,255,255,0.55)" }}>
            {title}
          </span>
...
export function RoomPlannerSidebar({ availableModels }: RoomPlannerSidebarProps) {
  const {
  ...
  // Find the available model config for the selected object (to get its color variants)
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 flex items-center justify-between shrink-0"
        ...
        <div>
          <h2 className="text-sm font-bold tracking-wide text-white">Oda Planlayıcı</h2>
      ...
      {/* ── Tools ── */}
      <Section title="Araçlar" icon={MousePointer}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${active ? "rgba(212,168,83,0.4)" : "rgba(255,255,255,0.06)"}`,
                  color: active ? "#D4A853" : "rgba(255,255,255,0.45)",
                }}
              >
        ...
        {selectedObj && (
          <div className="mt-3 rounded-xl overflow-hidden"
            ...
            {/* Color variants for selected object */}
                  ...
                  {selectedModel.colorVariants.map((cv) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: cv.hex,
                          border: active ? "2px solid #D4A853" : "2px solid rgba(255,255,255,0.12)",
                      ...
                      >
                        {active && (
                          <Check
                            ...
                            style={{
                              color: parseInt(cv.hex.slice(1), 16) > 0xaaaaaa ? "#000" : "#fff",
                            }}
                          />
                        )}
                      </button>
      ...
      {/* ── Room Dimensions ── */}
      <Section title="Oda Boyutları" icon={Ruler}>
          ...
          {[
            ...
            <div key={key}>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-[10px] font-medium" style={{ color: "rgba(255,255,255,0.45)" }}>{label}</label>
              ...
              <input
                ...
                style={{
                  background: `linear-gradient(90deg, #D4A853 ${((val - min) / (max - min)) * 100}%, rgba(255,255,255,0.1) 0%)`,
                  accentColor: "#D4A853",
                }}
              />
              <div className="flex justify-between mt-0.5">
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)" }}>{min}m</span>
                <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.2)" }}>{max}m</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── Appearance ── */}
      <Section title="Görünüm" icon={Paintbrush}>
        <div className="space-y-4">
          {/* Wall colors */}
          <div>
            <p className="text-[10px] font-medium mb-2.5" style={{ color: "rgba(255,255,255,0.4)" }}>Duvar Rengi</p>
            <div className="grid grid-cols-4 gap-1.5">
              {WALL_COLORS.map(({ hex, label }) => (
                <button
                  ...
                  style={{
                    background: hex,
                    border: wallColor === hex ? "2.5px solid #D4A853" : "2px solid rgba(255,255,255,0.1)",
                  ...
                  {wallColor === hex && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-1.5 h-1.5 rounded-full"
                        style={{ background: parseInt(hex.slice(1), 16) > 0xaaaaaa ? "rgba(0,0,0,0.6)" : "rgba(255,255,255,0.8)" }} />
                    </div>
                  )}
                </button>
              ))}
          ...
          {/* Floor textures */}
          <div>
            <p className="text-[10px] font-medium mb-2.5" style={{ color: "rgba(255,255,255,0.4)" }}>Zemin Malzeme</p>
              ...
              {FLOOR_TEXTURES.map(({ id, label, color }) => (
                <button key={id} title={label} onClick={() => setFloorTexture(id)}
                  ...
                  <div
                    ...
                    style={{
                      background: color,
                      border: floorTexture === id ? "2.5px solid #D4A853" : "2px solid rgba(255,255,255,0.08)",
                      transform: floorTexture === id ? "scale(1.08)" : "scale(1)",
                    }}
                  />
                  <span className="text-[9px] font-medium"
                    style={{ color: floorTexture === id ? "#D4A853" : "rgba(255,255,255,0.3)" }}>
                    {label}
                  </span>
                </button>
              ))}
            ...
            {availableModels.map((model) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-white truncate">{model.productName}</p>
            ...
            {objects.map((obj) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: sel ? "rgba(212,168,83,0.1)" : "rgba(255,255,255,0.03)",
                    border: `1px solid ${sel ? "rgba(212,168,83,0.3)" : "rgba(255,255,255,0.05)"}`,
                  }}
                >
                  {/* Color swatch if tinted */}
                  <div
                    className="w-2 h-2 rounded-full shrink-0"
                    style={{
                      background: obj.colorHex || "#D4A853",
                      opacity: sel ? 1 : 0.5,
                      border: "1px solid rgba(255,255,255,0.2)",
                    }}
                  />
                  <span className={cn("text-xs flex-1 truncate")}
                    style={{ color: sel ? "#D4A853" : "rgba(255,255,255,0.5)" }}>
                    {obj.productName}
                  </span>
                  <button
      ...
      {/* ── Actions ── */}
      <div className="mt-auto p-4 space-y-2 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
